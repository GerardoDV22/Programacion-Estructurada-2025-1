...
def evaluar_prestamo(indice):
    ...
    if int(cliente[2]) < 21 or int(cliente[2]) > 75: # 21 < Edad < 75
        return "Rechazado - Edad fuera de rango"
    if int(cliente[5]) < 1: #Estabilidad laboral < 1
        return "Rechazado - Poca estabilidad laboral"
    if int(cliente[6]) < 5000: #Ingreso Mensual < 5000
        return "Rechazado - Ingreso insuficiente"
    if float(cliente[4]) < (float(cliente[6]) * 0.25): #Saldo < Ingreso Mensual/4
        return "Rechazado - Bajo saldo"
    if int(cliente[7]) < 0: # Historial de prestamos
        return "Rechazado - Mal historial"
    if int(cliente[8]) > int(cliente[6])*2.5:
        return "Prestamos Rechazado - Adeudos Pendientes"

    monto_maximo = float(cliente[6]) * 2.5 #Ingreso * 2.5
    print(f"\nAprobado - Monto máximo: {monto_maximo:.2f} MXN")
    while True:
        try:
            prestamo = int(input("Ingrese el monto solicitado (Ingrese 0 para cancelar): "))
...
def abonar_prestamo(indice):
    while True:
        try:
            abono = int(input("\nIngresa el monto del abono (0 para cancelar): "))
            if abono == 0:
                            ...
                            try:
                                ...
                                while True:
                                    ...
                                    if confirmar.lower() == "y":
                                        nuevo_cliente = "\n"+str(cuenta)+","+nombre+","+str(edad)+","+str(PIN)+",0,"+str(estabilidad)+","+str(ingreso)+",0,0"
                                        with open("base_de_datos.txt","r+") as base_datos:
                                            contenido = base_datos.readlines()
